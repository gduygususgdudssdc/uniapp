...
-- 获取主分类ID并插入子分类
SET @cat_electronics = (SELECT id FROM categories WHERE name = '电子产品');
SET @cat_clothing = (SELECT id FROM categories WHERE name = '服装配饰');
SET @cat_home = (SELECT id FROM categories WHERE name = '家居用品');
SET @cat_books = (SELECT id FROM categories WHERE name = '图书文具');
SET @cat_sports = (SELECT id FROM categories WHERE name = '运动户外');
SET @cat_beauty = (SELECT id FROM categories WHERE name = '美妆护肤');
...
-- 获取用户ID和分类ID
SET @user1 = (SELECT id FROM users WHERE phone = '13800138001');
SET @user2 = (SELECT id FROM users WHERE phone = '13800138002');
SET @user3 = (SELECT id FROM users WHERE phone = '13800138003');
SET @user4 = (SELECT id FROM users WHERE phone = '13800138004');
SET @user5 = (SELECT id FROM users WHERE phone = '13800138005');

SET @cat_phone = (SELECT id FROM categories WHERE name = '手机');
SET @cat_computer = (SELECT id FROM categories WHERE name = '电脑');
SET @cat_camera = (SELECT id FROM categories WHERE name = '相机');
SET @cat_headphone = (SELECT id FROM categories WHERE name = '耳机');
SET @cat_male_clothing = (SELECT id FROM categories WHERE name = '男装');
SET @cat_female_clothing = (SELECT id FROM categories WHERE name = '女装');
SET @cat_shoes = (SELECT id FROM categories WHERE name = '鞋靴');
SET @cat_furniture = (SELECT id FROM categories WHERE name = '家具');
SET @cat_appliance = (SELECT id FROM categories WHERE name = '家电');
SET @cat_novel = (SELECT id FROM categories WHERE name = '小说');
SET @cat_sports_shoes = (SELECT id FROM categories WHERE name = '运动鞋');
SET @cat_skincare = (SELECT id FROM categories WHERE name = '护肤品');
...
-- 获取已售出商品ID
SET @goods_sold1 = (SELECT id FROM goods WHERE title = '小米 12S Ultra 256GB');
SET @goods_sold2 = (SELECT id FROM goods WHERE title = '联想 ThinkPad X1 Carbon');

-- 获取买家ID
SET @buyer1 = (SELECT id FROM users WHERE phone = '13800138004');
SET @buyer2 = (SELECT id FROM users WHERE phone = '13800138005');
...
('ORD20231110001', @goods_sold1, '小米 12S Ultra 256GB', 'https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=400&h=400&fit=crop', 3500.00, 1, 3500.00, @buyer1, @user3, 'completed', DATE_SUB(NOW(), INTERVAL 15 DAY), DATE_SUB(NOW(), INTERVAL 14 DAY), DATE_SUB(NOW(), INTERVAL 13 DAY), DATE_SUB(NOW(), INTERVAL 10 DAY)),
('ORD20231110002', @goods_sold2, '联想 ThinkPad X1 Carbon', 'https://images.unsplash.com/photo-1541807084-5c52b6b3adef?w=400&h=400&fit=crop', 4500.00, 1, 4500.00, @buyer2, @user4, 'completed', DATE_SUB(NOW(), INTERVAL 12 DAY), DATE_SUB(NOW(), INTERVAL 11 DAY), DATE_SUB(NOW(), INTERVAL 10 DAY), DATE_SUB(NOW(), INTERVAL 8 DAY)),
...
-- 获取商品ID
SET @goods1 = (SELECT id FROM goods WHERE title = 'iPhone 13 Pro 256GB 深空灰色');
SET @goods2 = (SELECT id FROM goods WHERE title = 'MacBook Pro 14寸 M1 Pro芯片');
SET @goods3 = (SELECT id FROM goods WHERE title = '佳能 EOS R6 全画幅微单相机');
SET @goods4 = (SELECT id FROM goods WHERE title = 'AirPods Pro 2代 降噪耳机');
SET @goods5 = (SELECT id FROM goods WHERE title = '华为 Mate 50 Pro 512GB');
